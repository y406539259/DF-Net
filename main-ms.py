...
import argparse
import time
from core.utils import calculate_Accuracy, get_img_list, get_model, get_data, get_data_structure
from pylab import *
import random
from test_ms import fast_test
...
# --------------------------------------------------------------------------------
models_list = ['UNet512', 'UNet512_sideoutput', 'M_Net', "FM_Net_SelfLearning"]
...
parser.add_argument('--my_description', type=str, default='ALL_DRIVE_TRAIN_1',
...
EPS = 1e-12
# define path
img_structure = get_img_list(Dataset, SubID, flag='train')
...
test_img_list = test_img_structure['Img']
...
for epoch in range(args.epochs):
    ...
    for i, (start, end) in enumerate(zip(range(0, len(img_list), args.batch_size),
        ...
        img, imageGreys, gt, tmp_gt, img_shape, label_ori, mask_ori= get_data_structure(Dataset, img_structure, index=0, batchsize=args.batch_size, img_size=args.img_size, gpu=args.use_gpu)
        #img = get_data_structure(Dataset, img_structure,img_size=args.img_size,gpu=args.use_gpu)
        #continue
    ...
    if True:
        ...
        Accuracy = fast_test(model, args, test_img_structure, model_name, epoch)
        print('BestAccuracy:', BestAccuracy)
        if Accuracy > BestAccuracy:
            BestAccuracy = Accuracy
            torch.save(model.state_dict(),
                      '%s/models/%s_%s/%s_%s.pth' % (RootDir, model_name, args.my_description,str(epoch), str(BestAccuracy)))
            # For evaluation
