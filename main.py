...
parser.add_argument('--my_description', type=str, default='DRIVE_PreTrain_1_0.001',
...
parser.add_argument('--gpu_avaiable', type=str, default='5',
                    help='the gpu used')
...
#Load the pretrained model，for multi source pretraining
LoadModel = 0
if LoadModel == 1:
    model_path = "/home/yinpengshuai/FrangiNetNew/models/FM_Net_SelfLearning_ALL_DRIVE_TRAIN_FINAL0/85_0.9621817.pth"
...
for epoch in range(args.epochs):
    ...
    if True:
        ...
        Accuracy = fast_test(model, args, test_img_list, model_name, epoch)
        print('BestAccuracy:', BestAccuracy)
        if Accuracy > BestAccuracy:
            BestAccuracy = Accuracy
            torch.save(model.state_dict(),
                       '%s/models/%s_%s/%s_%s.pth' % (RootDir, model_name, args.my_description, str(epoch), str(Accuracy)))
            # For evaluation
