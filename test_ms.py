...
import argparse
import time
from core.utils import calculate_Accuracy, get_model, get_data, get_img_list, get_data_structure
from pylab import *
import cv2
import warnings
...
# --------------------------------------------------------------------------------
models_list = ['UNet512', 'M_Net', "FM_Net_SelfLearning"]
dataset_list = ["All_DRIVE", "All_CHASEDB1", "All_STARE"]
# --------------------------------------------------------------------------------
...
parser.add_argument('--SubImageID', type=int, default=0,
...
parser.add_argument('--my_description', type=str, default='testStare',
...
def fast_test(model, args, img_structure, model_name, epoch):
    ...
    for i, path in enumerate(img_list):
        ...
        img, imageGreys, gt, tmp_gt, img_shape, label_ori, mask_ori = get_data_structure(Dataset, img_structure,
                                                                                         index=i,
    ...
    print('Epoch: %s |  Acc: %s  |  Se: %s |  Sp: %s |  Auc: %s |  Background_IOU: %s |  vessel_IOU: %s '%(str(epoch), str(np.mean(np.stack(ACC))), str(np.mean(np.stack(SE))), str(np.mean(np.stack(SP))), str(np.mean(np.stack(AUC))),str(np.mean(np.stack(Background_IOU))),str(np.mean(np.stack(Vessel_IOU)))))

    # store test information
    with open(r'/home/yinpengshuai/FrangiNetNew/logs/%s_%s.txt' % (model_name, args.my_description), 'a+') as f:
        f.write('Epoch: %s |  Acc: %s  |  Se: %s |  Sp: %s |  Auc: %s |  Background_IOU: %s |  vessel_IOU: %s '%(str(epoch), str(np.mean(np.stack(ACC))), str(np.mean(np.stack(SE))), str(np.mean(np.stack(SP))), str(np.mean(np.stack(AUC))),str(np.mean(np.stack(Background_IOU))),str(np.mean(np.stack(Vessel_IOU)))))
        f.write('\n\n')
...
if __name__ == '__main__':
    ...
    if args.use_gpu:
        model.cuda()
    if True:
        #model_path = 'D:\\FrangiNet\\FrangiNet\\models\\FM_Net_SelfLearning_M-Net\\75.pth'
        #model_path = 'D:\\FrangiNet\\FrangiNet\\models\\UNet512_DRIVE-WithOutSide-Output\\100.pth'
        #model_path = 'D:\\FrangiNet\\FrangiNet\\models\\M_Net_DRIVE-WithOutSide-Output\\35.pth'
        #model_path = 'D:\\FrangiNet\\FrangiNet\\models\\M_Net_DRIVE\\20.pth'
        #model_path = 'D:\\FrangiNet\\FrangiNet\\models\\FM_Net_SelfLearning_CHASEDB1\\30.pth'
        #model_path = "D:\\FrangiNet\\models\\FM_Net_SelfLearning_CHASEDB1-DataAug\\60.pth"
        #model_path = "D:\\FrangiNet\\models\\SU_Net_DRIVE-300\\105.pth"
        #model_path = "D:\\FrangiNet\\models\\FM_Net_SelfLearning_CHASEDB1-DataAug\\195.pth"
        #model_path = "D:\\FrangiNet\\models\\SU_Net_CHASEDB1-sunet\\65.pth"
        #model_path = "D:\\FrangiNet\\models\\FM_Net_SelfLearning_DRIVE1-DataAug-R3\\185.pth"
        #model_path = "/home/yinpengshuai/FrangiNetNew/models/FM_Net_SelfLearning_ALL_DRIVE_Train/8.pth"
        #model_path  = "/home/yinpengshuai/FrangiNetNew/models/FM_Net_SelfLearning_ALL_DRIVE_Train3/195.pth"
        #model_path = "/home/yinpengshuai/FrangiNetNew/models/FM_Net_SelfLearning_ALL_CHASEDB1_Train/33.pth"

        #model_path = "/home/yinpengshuai/FrangiNetNew/models/FM_Net_SelfLearning_ALL_DRIVE_Final/129_0.965356.pth"
        model_path = "/home/yinpengshuai/FrangiNetNew/models/FM_Net_SelfLearning_ALL_STARE1_Final_0/99_0.9752436.pth"
    ...
    #test_img_structure = get_data_structure(Dataset, SubID, flag='test')
    test_img_structure = get_img_list(Dataset, SubID, flag='test')

    fast_test(model, args, test_img_structure, model_name, 0)
